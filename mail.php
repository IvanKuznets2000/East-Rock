...
foreach ( $_POST as $key => $value ) {
	if ( $value != ""  && $key != "admin_email" && $key != "form_subject" ) {
		...
		$message .= "
		" . ( ($c = !$c) ? '<tr>':'<tr>' ) . "
		<td style='padding: 5px; width: auto;'><b>$key:</b></td>
		<td style='padding: 5px;width: 100%;'>$value</td>
		</tr>
		";
	}
}
$message = "<table style='width: 70%;'>$message</table>";
...
if ($_FILES){
	foreach ( $_FILES['files']['tmp_name'] as $key => $value ) {
		$mail->addAttachment($value, $_FILES['files']['name'][$key]);
	}
}
$mail->send();
?>